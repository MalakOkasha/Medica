...
export default function viewAllCompanies() {
  ...
  const fetchCompanies = () => {
    fetch('http://localhost:8082/api/pharma/all')
      ...
      .catch(() => {
        setErrorMessage('Error loading company data');
        setLoading(false);
      });
  };
...
const handleDelete = (id) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      fetch(`http://localhost:8082/api/pharma/delete/${id}`, {
        method: 'DELETE',
        headers: {
  ...
  const handleEdit = (id) => {
    ...
    router.push(`/admin/editCompany?id=${encodedId}`);
  };

  const filteredCompanies = companies.filter((company) =>
    company.user.fullName.toLowerCase().includes(search.toLowerCase()) ||
    company.user.email.toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-b from-white to-blue-100 px-4 py-6 ${outfit.className}`}>
      ...
      <header className="flex items-center justify-between px-4 py-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 font-medium">View all Companies</p>
          </div>
        </div>
      ...
      <div className="hidden md:flex bg-white/90 backdrop-blur-md border border-white/30 rounded-2xl shadow-md px-6 py-4 mb-3">
        <div className="flex-1 text-blue-950 text-lg font-bold">Name</div>
        <div className="flex-1 text-blue-950 text-lg font-bold">Email</div>
        <div className="flex-1 text-blue-950 text-lg font-bold">Phone</div>
        <div className="flex-1 text-blue-950 text-lg font-bold">Location</div>
      ...
      <div className="flex flex-col gap-3">
        {loading ? (
          <div className="w-full text-center text-gray-600 font-semibold text-lg mt-6">Loading companies...</div>
        ) : (
          filteredCompanies.map(({ id, user, location }) => (
            <div
              key={id}
            ...
            >
              <div className="flex-1 text-black text-base md:text-lg font-medium">{user.fullName}</div>
              <div className="flex-1 text-black text-base md:text-lg font-medium">{user.email}</div>
              <div className="flex-1 text-black text-base md:text-lg font-medium">{user.contactInfo}</div>
              <div className="flex-1 text-black text-base md:text-lg font-medium">{location}</div>
