...
export default function EditAdmin() {
  ...
  useEffect(() => {
    if (!adminId || !isAuthorized) return;
    setLoading(true);

    fetch(`http://localhost:8082/api/auth/admin/${adminId}`)
      .then((res) => {
  ...
  const handleUpdate = async () => {
    try {
      const res = await fetch(`http://localhost:8082/api/auth/admin/edit/${adminId}`, {
        method: 'PUT',
      ...
      if (res.ok) {
        ...
        if (contentType?.includes('application/json')) {
          ...
          Swal.fire({
            ...
            text: errorData.message || 'Failed to update admin',
          });
        } else {
    ...
    <div className="min-h-screen bg-gradient-to-b from-white to-blue-100 px-6 py-12 font-sans">
      ...
      <div className="flex items-center gap-4 mb-10">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600 font-medium">Edit Admin</p>
        </div>
      </div>
      ...
      <div className="flex justify-center mt-10">
        <button
          ...
          type="button"
          className="w-40 h-14 bg-blue-300 rounded-full text-black font-bold shadow hover:bg-blue-400 transition-colors duration-300 cursor-pointer z-10"
        >
          Save
        </button>
      </div>
